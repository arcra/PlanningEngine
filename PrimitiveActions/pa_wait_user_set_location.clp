...
(defrule wait_user_set_location-send_speech
	?p <-(plan (task ?taskName) (action_type wait_user_set_location) (params ?location) (step $?steps) (parent ?pp))
	(active_plan ?p)
	(not
		(plan_status ?p ?)
	)
	...
	=>
	(assert
		(plan (task ?taskName) (action_type spg_say) (params "Please set the location" ?location " in the map, so I can continue with the execution.") (step $?steps) (parent ?pp))
		(speech_notification_sent wait_user_set_location)
	)
