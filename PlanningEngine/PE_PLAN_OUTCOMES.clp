...
(defrule successful_task-delete_successful_task ; There is a task with same hierarchy (unordered or successor) that should be executed. Delete this (successful) task so the others can be activated.
	...
	=>
	(retract ?ts ?at ?task)
	(log-message INFO "Successful task of plan '" ?planName "' with action_type: '" ?action_type "' and params: '" (implode$ $?params) "' was deleted. Other tasks of same hierarchy (unordered or successors) exist.")
)
...
(defrule successful_task-mark_children_successful ; When there are no tasks with same hierarchy (unordered or successors), mark exeuction as successful (so the parent task can decide whether to decompose again or mark itself as successful)
	...
	(log-message INFO "Successful task of plan '" ?planName "' with action_type: '" ?action_type "' and params: '" (implode$ $?params) "' was deleted with no other tasks of same hierarchy. Parent task with action_type: '" ?action_type_PP "' and params: '" (implode$ $?params_PP) "' is now active.")
)
...
(defrule successful_task-top_level_succeeded ; LAST Top-level task for this plan was successful
	...
	=>
	(retract ?ts ?at ?task)
	(log-message INFO "Top level task of plan '" ?planName "' with action_type: '" ?action_type "' and params: '" (implode$ $?params) "' succeeded!")
	(assert
...
(defrule failed_task-delete_tasks_with_same_hierarchy ; Other tasks with same hierarchy (unordered or successors) must be cleared out.
	...
	=>
	(retract ?task2)
	(log-message WARNING "Plan '" ?action_type "' with steps: '" ?step " " (implode$ $?steps) "' failed. Deleted same hierarchy task for plan '" ?planName "' with action_type: '" ?action_type2 "' and params: '" (implode$ $?params2) "'.")
)
...
(defrule failed_task-delete_failed_task-mark_children_status ; After removing all other same-hierarchy tasks, remove this task, set children_status as failed, activate parent task and let the engine replan. (this rule applies to non-top-level tasks)
	...
	(log-message WARNING "Failed task of plan '" ?planName "' with action_type: '" ?action_type "' and params: '" (implode$ $?params) "' was deleted. Parent task activated for replanning.")
)
